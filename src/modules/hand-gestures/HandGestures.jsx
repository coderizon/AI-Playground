...
import { drawHandLandmarks } from './drawHandLandmarks.js';
...
export default function HandGestures() {
  ...
  const handQueueRef = useRef(Promise.resolve());
  const pendingDetectionsRef = useRef(0);
  const lastHandRef = useRef({ hands: [], features: null });
  ...
  const enqueueHandDetection = useCallback(
    (input, { allowSkip = false, updatePreview = true } = {}) => {
      if (!input) return Promise.resolve({ hands: [], features: null });

      if (allowSkip && pendingDetectionsRef.current > 0) {
        return Promise.resolve(lastHandRef.current ?? { hands: [], features: null });
      }
      ...
      const task = async () => {
        try {
          const result = await getHandFeatures(input);
          lastHandRef.current = result ?? { hands: [], features: null };
          if (updatePreview) {
            setActiveHands(result?.hands ?? []);
          }
          return result;
        } finally {
          pendingDetectionsRef.current -= 1;
      ...
      const next = handQueueRef.current.then(task, task);
      handQueueRef.current = next.catch(() => {});
      return next;
    },
    [getHandFeatures],
  );

  const extractHandFeatures = useCallback(
    async (input) => {
      const isLiveInput = Boolean(input && typeof input.videoWidth === 'number');
      const { features } = await enqueueHandDetection(input, {
        allowSkip: false,
        updatePreview: isLiveInput,
      });
      return features;
    },
    [enqueueHandDetection],
  );
  ...
  const modelStatusMessage = useMemo(() => {
    if (handStatus === 'loading') {
      return 'Modell wird geladen. Gestenerkennung ist gleich verfügbar.';
    }
    if (handStatus === 'error') {
  ...
  useEffect(() => {
    ...
    if (handStatus !== 'ready') return undefined;
    if (activeStep === 'test') return undefined;
    ...
    const loop = async () => {
      ...
      if (videoEl?.readyState >= 2) {
        try {
          await enqueueHandDetection(videoEl, { allowSkip: true, updatePreview: true });
        } catch (error) {
          console.error(error);
        }
      }
  ...
  }, [activeStep, enqueueHandDetection, handStatus, shouldEnableWebcam]);
  ...
  const {
    ...
    extractFeatures: extractHandFeatures,
    featureSize: outputDim,
    videoRef: captureVideoRef,
    modelStatus: handStatus,
    ...
    <div className={styles['image-classification']}>
      ...
      <div className={styles['ic-shell']}>
        ...
        <nav
          ...
          aria-label="Gestenerkennung Schritte"
          style={{
            ...
            <section className={styles['classes-column']}>
              ...
              {classes.map((cls, index) => (
                <ClassCard
                  ...
                  overlayRenderer={drawHandOverlay}
